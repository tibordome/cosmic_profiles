...
def getDensProfsDirectBinning(float[:,:] xyz, int[:] obj_keep, float[:] masses, float[:] r200s, float[:] ROverR200, cat, int MIN_NUMBER_PTCS, float L_BOX, str CENTER):
...
@cython.embedsignature(True)
@np_cache_factory(5,1)
def getDensProfsKernelBased(float[:,:] xyz, int[:] obj_keep, float[:] masses, float[:] r200s, float[:] ROverR200, cat, int MIN_NUMBER_PTCS, float L_BOX, str CENTER):
